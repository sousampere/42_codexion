...
/*   Updated: 2026/04/09 14:23:14 by gaspard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	coder->left_dongle->cooldown = get_rel_time(mng) + mng->args->dongle_cooldown;
	coder->right_dongle->cooldown = get_rel_time(mng) + mng->args->dongle_cooldown;
...
void	refactor(t_coder *coder, t_manager *mng)
{
	...
	usleep(mng->args->refactor_time * 1000); // simulate refactor
}
